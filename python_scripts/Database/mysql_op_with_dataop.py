...
class MySql_Operation():
    def __init__(self, db_nick_name='mysql00', db_name = None):
        self.__conn = DataOp.GetDBConnect(db_nick_name)
    ...
    def execute_sql(self, sql, param = (), bSSCursor = False):
        ...
        try:
            ...
            print "Mysql Error %s: %s, sql is %s" % (e.args[0], e.args[1], sql)
        except Exception, e:
            print "Mysql Error %s, sql is %s" % (e, sql)
    # end of def execute_sql(self, sql, param = ())


    def traverse_table_with_dict_result(self, sql_cmd, size=5000):
