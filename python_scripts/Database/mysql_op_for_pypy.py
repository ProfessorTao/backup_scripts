...
class MySql_Operation():
    ...
    def __get_conn(self):
        try:
            #self.__conn = pymysql.connect(host=self.__host, user=self.__user, passwd=self.__passwd, db=self.__name, port=self.__port, charset="utf8", local_infile = 1)
            self.__conn = pymysql.connect(host=self.__host, user=self.__user, passwd=self.__passwd, db=self.__name, port=self.__port, charset="utf8")
            self.__cur = self.__conn.cursor()
    ...
    def fetchmany(self, sql_cmd, size=5000):
        cursor = self.__conn.cursor(pymysql.cursors.SSDictCursor) 
    ...
    def traverse_table_with_sscursor(self, sql):
        ...
        self.__sscur = self.__conn.cursor(pymysql.cursors.SSCursor)
